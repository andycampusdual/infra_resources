...
resource "aws_subnet" "subnet_a" {
  ...
  cidr_block        = "172.31.253.0/24"  # Cambiar CIDR block
  availability_zone = "eu-west-3a"
}

resource "aws_subnet" "subnet_b" {
  vpc_id            = aws_default_vpc.default_vpc.id
  cidr_block        = "172.31.254.0/24"  # Cambiar CIDR block
  availability_zone = "eu-west-3b"
}

resource "aws_subnet" "subnet_c" {
  vpc_id            = aws_default_vpc.default_vpc.id
  cidr_block        = "172.31.255.0/24"  # Cambiar CIDR block
  availability_zone = "eu-west-3c"
}

resource "aws_ecr_repository" "mi_primer_ecr_repo" {
  name = "mi-primer-ecr-repo-agd-v4"  # Cambiar nombre
}

resource "aws_ecs_cluster" "mi_cluster" {
  name = "agd-cluster-new-v4"  # Cambiar nombre
}

resource "aws_iam_role" "ecsTaskExecutionRoleNew_agd" {
  name               = "ecsTaskExecutionRoleNew_agd_v4"  # Cambiar nombre
...
resource "aws_secretsmanager_secret" "db_credentials_main" {
  name = "db_credentials_main_v4"  # Cambiar nombre
}
...
resource "aws_db_subnet_group" "main" {
  name       = "main-subnet-group-v4"  # Cambiar nombre
...
resource "aws_ecs_task_definition" "mi_primer_task" {
  family                   = "mi-primer-task-agd-v4"  # Cambiar nombre
  container_definitions    = <<DEFINITION
  [
    {
      "name": "mi-primer-task-agd-v4",
...
resource "aws_alb" "application_load_balancer" {
  name               = "test-lb-tf-v4"  # Cambiar nombre
...
resource "aws_lb_target_group" "target_group" {
  name        = "target-group-v4"  # Cambiar nombre
  port        = 80
  protocol    = "HTTP"
  target_type = "ip"
  vpc_id      = "${aws_default_vpc.default_vpc.id}" 
...
resource "aws_ecs_service" "mi_primer_service_agd" {
  name            = "mi-primer-service-agd-v4"  # Cambiar nombre
...
resource "aws_appautoscaling_policy" "ecs_scaling_policy_main" {
  name               = "scale-out-main-v4"  # Cambiar nombre
...
resource "aws_cloudwatch_metric_alarm" "ecs_cpu_high_main" {
  alarm_name          = "ecs-cpu-high-main-v4"  # Cambiar nombre
...
resource "aws_cloudwatch_metric_alarm" "ecs_cpu_low_main" {
  alarm_name          = "ecs-cpu-low-main-v4"  # Cambiar nombre
